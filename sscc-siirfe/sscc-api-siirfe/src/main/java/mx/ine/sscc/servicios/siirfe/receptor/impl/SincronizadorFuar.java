 ...
 * contiene la unidad de trabajo necesaria para la sincronización de la tabla FUAR.]
 * @author Orlando Adrián Ramos Galván (orlandoa.ramos@outlook.com)
 * @version 1.0
 * @since SIIRFE 5.10
 */
@Component("SincronizadorFuar")
public class SincronizadorFuar implements Callable<Object>, Sincronizador {

    private static final Logger logger = LoggerFactory.getLogger(SincronizadorFuar.class);
    ...
    public Object call() {
        ...
        do {
            try {
                ...
                bitacoraManager.creaBitacoraPrincipal(TipoTablaActualizacion.FUAR.getClave());
                ...
                for (Entry<Long, DatosSalida> datosRespuesta : respuestaSincronizacion
                    ...
                    for (DatosRegistro datosRegistro : datosRespuesta.getValue()
                        ...
                        if (TipoControlSincronizacion.REPROCESADO.getClave()
                            ...
                            fuarService.actualizaFuar(datosRegistro.getId(),
    ...
    public void setThreadName() {
        Thread.currentThread()
            .setName(GeneralConstants._BLOQUE_FUAR + Thread.currentThread().getId());
    }
    ...
    public synchronized boolean obtieneBloque(DatosEntrada datosEntrada) {
        ...
        /*
         * OBTIENE REGISTROS FUAR
         */
        try {
            setBitacoraId(bitacoraManager.getBitacoraMaster().getProcesoSincronizacionId());
            datosEntrada.setFuarSiirfe(fuarService.obtieneListaFuar(getBitacoraId()));
            datosEntrada.setProcesoSincronizacioId(getBitacoraId());
            setRegBloque(datosEntrada.getFuarSiirfe().size());
    ...
    public synchronized boolean enviaBloque(DatosEntrada datosEntrada) {
         ...
         */
        try {
            setRespuestaSincronizacion(actualizaCliente.obtieneRespuestaFuar(datosEntrada));
    ...
    public synchronized void actualizaBloqueError(DatosEntrada datosEntrada) {
        if (datosEntrada != null) {
            fuarService.actualizaBloqueFuar(TipoControlSincronizacion.PENDIENTE.getClave(),
    ...
    public synchronized void actualizaBloqueExito(Long bitacoraId) {
        // TODO [codificar el cuerpo del método]
        fuarService.actualizaBloqueFuar(TipoControlSincronizacion.TERMINADO.getClave(),
            bitacoraId);
    }
