...
import mx.ine.sscc.servicios.siirfe.observador.FuarRechObservador;
...
/**
 * TODO [Esta clase tiene la responsabilidad de voltear a "ver" la tabla Fuar Rechazado cada
...
@Component
@EnableScheduling
public class SchedulerFuarRechObservable extends Observable implements Sujeto {

    private static final Logger logger =
        LoggerFactory.getLogger(SchedulerFuarRechObservable.class);

    @Resource
    private FuarRechObservador fuarRechObservador;

    @Resource
    private ExecutorServicesManagment executorServicesManagment;
    private ExecutorService executorService;

    private Long registrosFuarRech;
     ...
     * @author Orlando Adrián Ramos Galván,(orlandoa.ramos@outlook.com)
     */
    public SchedulerFuarRechObservable() {
    ...
    private void init() {
        ...
        addObserver(fuarRechObservador);
    ...
    public void verificaRegistros() {
        _isON();
        Future<Long> _registrosFuarRech;
        if (GeneralConstants.ON.equals(io_btn)) {

            logger.info(">>> FUAR RECHAZADO");
            _registrosFuarRech = executorService.submit(miraTablaFuarRech());
            try {
                logger.info(">>> Cantidad de registros: " + _registrosFuarRech.get());
                setRegistrosFuarRech(_registrosFuarRech.get());
    ...
    public void notificaObservador() {
        ...
        if (getRegistrosFuarRech() > 0) {
            setChanged();
            notifyObservers(TipoTablaActualizacion.FUAR_RECHAZADO.getId());
        }
    }
     ...
     */
    private Callable<Long> miraTablaFuarRech() {
        // TODO [codificar el cuerpo del método]
    ...
    private void _isON() {
        ...
        try {
            setIo_btn(prop.getProperty("siirfe.sincro.fuarrech"));
        } catch (Exception e) {
            // TODO: handle exception
            logger.error("**** Error ****");
            setIo_btn("off");
     ...
     */
    public Long getRegistrosFuarRech() {
        return registrosFuarRech;
    }

    /**
     * @param registrosFuarRech parametro registrosFuarRech a actualizar
     */
    public void setRegistrosFuarRech(Long registrosFuarRech) {
        this.registrosFuarRech = registrosFuarRech;
        notificaObservador();
    }
