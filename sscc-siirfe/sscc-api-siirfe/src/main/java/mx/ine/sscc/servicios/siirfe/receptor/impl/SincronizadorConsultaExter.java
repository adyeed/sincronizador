 ...
 * contiene la unidad de trabajo necesaria para la sincronización de la tabla CONSULTA
 * EXTRANGERO.]
 * @author Orlando Adrián Ramos Galván (orlandoa.ramos@outlook.com)
 * @version 1.0
 * @since SIIRFE 5.10
 */
@Component("SincronizadorConsultaExter")
public class SincronizadorConsultaExter implements Callable<Object>, Sincronizador {

    private static final Logger logger =
        LoggerFactory.getLogger(SincronizadorConsultaExter.class);
     ...
     * @author Orlando Adrián Ramos Galván,(orlandoa.ramos@outlook.com)
     */
    public SincronizadorConsultaExter() {
    ...
    public Object call() {
        ...
        do {
            try {
                ...
                bitacoraManager
                    .creaBitacoraPrincipal(TipoTablaActualizacion.CONSULTAEXTERNA.getClave());
                ...
                logger.error("**** Error en bitacora principal " + " - " + e.getMessage()
                    + " - " + e.getCause().toString());
                logger.warn("°°°°°°°°°° EXCEPCION DETECTADA °°°°°°°°°°");
                ...
                for (Entry<Long, DatosSalida> datosRespuesta : respuestaSincronizacion
                    ...
                    for (DatosRegistro datosRegistro : datosRespuesta.getValue()
                        ...
                        if (TipoControlSincronizacion.REPROCESADO.getClave()
                            ...
                            consultaExternaService.actualizaConsultaExtr(datosRegistro.getId(),
    ...
    public void setThreadName() {
        Thread.currentThread().setName(
            GeneralConstants._BLOQUE_CONSULTAEXTERNA + Thread.currentThread().getId());
    }
    ...
    public synchronized boolean obtieneBloque(DatosEntrada datosEntrada) {
        ...
        logger.info(">> obtiene bloque de registros -- "
            + bitacoraManager.getBitacoraMaster().getProcesoSincronizacionId());
        // Se crea el id de la bitacora ficcticia.
        /*
         * OBTIENE REGISTROS Consulta Extr
         */
        try {
            setBitacoraId(bitacoraManager.getBitacoraMaster().getProcesoSincronizacionId());
            datosEntrada.setConsultaExternaSiirfe(
                consultaExternaService.obtieneListaConsultaExtr(getBitacoraId()));
            setRegBloque(datosEntrada.getConsultaExternaSiirfe().size());
        ...
        // FIN OBTIENE REGISTROS Consulta extr
    }
    ...
    public synchronized boolean enviaBloque(DatosEntrada datosEntrada) {
        ...
        try {
            setRespuestaSincronizacion(
                actualizaCliente.obtieneRespuestaConsulta(datosEntrada));
    ...
    public synchronized void actualizaBloqueError(DatosEntrada datosEntrada) {
        if (datosEntrada != null) {
            consultaExternaService.actualizaBloqueConsultaExtr(
    ...
    public synchronized void actualizaBloqueExito(Long bitacoraId) {
        // TODO [codificar el cuerpo del método]
        consultaExternaService.actualizaBloqueConsultaExtr(
            TipoControlSincronizacion.TERMINADO.getClave(), bitacoraId);
    }
