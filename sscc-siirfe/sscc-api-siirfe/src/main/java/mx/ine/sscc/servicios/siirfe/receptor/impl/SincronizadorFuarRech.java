...
/**
 * TODO [Clase que recibe la peticion del invocador.Esta clase contiene la unidad de trabajo
 * necesaria para la sincronización de la tabla FUAR RECHAZADO.]
 *
 * @author Orlando Adrián Ramos Galván (orlandoa.ramos@outlook.com)
 * @version 1.0
 * @since SIIRFE 5.10
 */
@Component("SincronizadorFuarRech")
public class SincronizadorFuarRech implements Callable<Object>, Sincronizador {

    private static final Logger logger = LoggerFactory.getLogger(SincronizadorFuarRech.class);

    private DatosEntrada datosEntrada;
    private RespuestaSincronizacion respuestaSincronizacion;
    @Resource
    private FuarRechService fuarRechService;
    @Resource
    ...
    public Object call() {
        // TODO [codificar el cuerpo del método]
        /*
         * Método principal, en este método se encuentra la unidad de trabajo
         * principal para la sincronziación de cada tabla.
         */
        setThreadName();
        ...
        do {
            try {
                ...
                bitacoraManager
                    .creaBitacoraPrincipal(TipoTablaActualizacion.FUAR_RECHAZADO.getClave());
    ...
    public void procesaRespuesta(RespuestaSincronizacion respuestaSincronizacion) {
        ...
        if (respuestaSincronizacion.getMensajeError() == null) {
            ...
            try {
                ...
                for (Entry<Long, DatosSalida> datosRespuesta : respuestaSincronizacion
                    ...
                    for (DatosRegistro datosRegistro : datosRespuesta.getValue()
                        ...
                        if (TipoControlSincronizacion.REPROCESADO.getClave()
                            ...
                            fuarRechService.actualizaFuarRech(datosRegistro.getId(),
                ...
                actualizaBloqueExito(getBitacoraId());
    ...
    public void setThreadName() {
        Thread.currentThread()
            .setName(GeneralConstants._BLOQUE_FUARRECHAZADO + Thread.currentThread().getId());
    }
    ...
    public synchronized boolean obtieneBloque(DatosEntrada datosEntrada) {
        ...
        try {
            ...
            datosEntrada.setFuarRechazadoSiirfe(
                fuarRechService.obtieneListaFuarRechazado(getBitacoraId()));
            datosEntrada.setProcesoSincronizacioId(getBitacoraId());
            setRegBloque(datosEntrada.getFuarRechazadoSiirfe().size());
            bitacoraManager.creaSubBitacoraExtraccion(getRegBloque());
            return true;
        } catch (Exception e) {
            // TODO: handle exception
            logger.error("error al obtener registros --> " + Thread.currentThread().getName()
                + " - " + e.getMessage() + " - " + e.getCause().toString());
    ...
    public synchronized boolean enviaBloque(DatosEntrada datosEntrada) {
        ...
        try {
            setRespuestaSincronizacion(
                actualizaCliente.obtieneRespuestaFuarRech(datosEntrada));
            bitacoraManager.creaSubBitacoraEnvioUnicom(getRegBloque());
            return true;
        } catch (WebServiceException e) {
            // TODO: handle exception
            logger.error(
                "*** error al consumir ws --> " + datosEntrada.getProcesoSincronizacioId()
                    + " - " + e.getMessage() + " - " + e.getCause().toString());
            actualizaBloqueError(datosEntrada);
     ...
     *
     * @see
     * mx.ine.sscc.servicios.siirfe.receptor.Sincronizador#actualizaBloqueExito(
     * )
     */
    @Override
    public synchronized void actualizaBloqueError(DatosEntrada DatosEntrada) {
        fuarRechService.actualizaBloqueFuarRech(TipoControlSincronizacion.PENDIENTE.getClave(),
    ...
    public synchronized void actualizaBloqueExito(Long bitacoraId) {
        // TODO [codificar el cuerpo del método]
        fuarRechService.actualizaBloqueFuarRech(TipoControlSincronizacion.TERMINADO.getClave(),
            bitacoraId);
    }
     ...
     * lo declara (non-Javadoc)
     *
     * @see mx.ine.sscc.servicios.siirfe.receptor.Sincronizador#
     * obtieneRegistrosSincronizables()
     */
    ...
    public Long obtieneRegistrosSincronizables() {
        // TODO [codificar el cuerpo del método]
        try {
            return fuarRechService.getAvailableRecordsFuarRech();
        } catch (Exception e) {
            // TODO: handle exception
