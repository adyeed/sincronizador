 ...
 * contiene la unidad de trabajo necesaria para la sincronización de la tabla ANALISIS
 * REGISTRAL.]
...
@Component("SincronizadorAnalisisReg")
public class SincronizadorAnalisisReg implements Callable<Object>, Sincronizador {

    private static final Logger logger =
        LoggerFactory.getLogger(SincronizadorAnalisisReg.class);
     ...
     * @author Orlando Adrián Ramos Galván,(orlandoa.ramos@outlook.com)
     */
    public SincronizadorAnalisisReg() {
    ...
    public Object call() {
        ...
        do {
            try {
                ...
                bitacoraManager.creaBitacoraPrincipal(
                    TipoTablaActualizacion.ANALISISREGISTRAL.getClave());
                ...
                for (Entry<Long, DatosSalida> datosRespuesta : respuestaSincronizacion
                    ...
                    for (DatosRegistro datosRegistro : datosRespuesta.getValue()
                        ...
                        if (TipoControlSincronizacion.REPROCESADO.getClave()
                            ...
                            analisisRegService.actualizaAnalisisReg(datosRegistro.getId(),
    ...
    public void setThreadName() {
        Thread.currentThread().setName(
            GeneralConstants._BLOQUE_ANALISISREGISTRAL + Thread.currentThread().getId());
    }
    ...
    public synchronized boolean obtieneBloque(DatosEntrada datosEntrada) {
        ...
        /*
         * OBTIENE REGISTROS Analisis Reg  */
        try {
            setBitacoraId(bitacoraManager.getBitacoraMaster().getProcesoSincronizacionId());
            datosEntrada.setAnalisisRegistralSiirfe(
                analisisRegService.obtieneListaAnalisisReg(bitacoraId));
            setRegBloque(datosEntrada.getAnalisisRegistralSiirfe().size());
        ...
        // FIN OBTIENE REGISTROS Analisis Reg
    }
    ...
    public synchronized boolean enviaBloque(DatosEntrada datosEntrada) {
        ...
        try {
            setRespuestaSincronizacion(
                actualizaCliente.obtieneRespuestaAnalisis(datosEntrada));
    ...
    public synchronized void actualizaBloqueError(DatosEntrada datosEntrada) {
        if (datosEntrada != null) {
            analisisRegService.actualizaBloqueAnalisisReg(
